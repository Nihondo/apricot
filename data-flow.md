...
10. [Web UI URL プレビューの流れ](#web-ui-url-プレビューの流れ)
...
┌──────────────────────▼───────────────────────────────────┐
│  Durable Object: IrcProxyDO (workers/src/irc-proxy.ts)                                                             │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ インメモリ状態                                                                                           │    │
│  │  - serverConn (TCP)    - clients (WebSocket set)                                                         │    │
│  │  - channelStates       - nick / channels                                                                 │    │
│  │  - web store (web-store.ts) - webUpdateSubscribers                                                       │    │
│  └─────────────────────────────────────────────────────┘    │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ DO Storage (永続化)                                                                                      │    │
...
│  ┌─────────────────────────────────────────────────────┐    │
│  │ モジュール / helper                                                                                      │    │
│  │  ping → web(web-module.ts) → channelTrack → clientSync                                                │    │
│  │  API/Web/auth/settings は workers/src/irc-proxy/*.ts に分割                                              │    │
│  └─────────────────────────────────────────────────────┘    │
└──────────────────────┬───────────────────────────────────┘
...
補足:
現在の実装では、DO 本体の高レベル制御は [workers/src/irc-proxy.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/irc-proxy.ts) に残しつつ、HTTP ハンドラや認証・設定処理は [workers/src/irc-proxy/](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/irc-proxy/) 配下へ分割しています。Web UI も [workers/src/modules/web-module.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/modules/web-module.ts), [workers/src/modules/web-store.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/modules/web-store.ts), [workers/src/modules/web-render.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/modules/web-render.ts), [workers/src/modules/web-theme.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/modules/web-theme.ts) に責務分割されています。
...
| `web:logs:v1` | `Record<string, PersistedStoredMessage[]>` | チャンネルメッセージ履歴（各行に `sequence` を含む） | メッセージ追加ごと + サーバ切断時 |
...
| `channelSequences` | チャネルごとの最新ログ sequence キャッシュ |
...
```
  ...
  ├─ cloudflare:sockets connect(host, port)
  ├─ socket.opened を待って TCP 確立確認
  ...
  └─ USER alice 0 * :apricot IRC Proxy
       │
       ▼
  サーバ応答: 020 などの pre-registration numeric / NOTICE
       │
       └─ registration timeout を延長しながら待機
       │
       ▼
...
```
  ...
  └─ 新しいメッセージ到着時は webUpdateSubscribers に最新 sequence 付き channel-updated を通知
       │
       ▼
  ブラウザ: /web/#general/messages/fragment?since=<latestSequence> を Fetch
  ├─ `since=0` または継続不能時は full 断片を返す
  ├─ `since>0` かつ継続可能なら delta 断片を返す
  └─ クライアントは sequence 整合時だけ DOM 追加し、異常時は全量置換
       │
       ▼
  再デプロイなどで半死に接続になった場合
  ├─ ping に対する pong / channel-updated が返らない
  ├─ 2回連続失敗で refreshMessages() を 1回だけ実行
  ├─ その直後に新しい /updates ソケットへ即時 reconnect
  ├─ reconnect 後も通常どおり `since=<latestSequence>` 付きで fragment を取得
  └─ sequence 不整合や full 応答時は全量置換して通常同期に復帰
```
...
```
  ...
  ├─ TCP connect / socket.opened 待ち
  ├─ PASS / NICK / USER 送信
  ├─ 登録前 numeric を待機
  │    ├─ 020 などの pre-registration 応答受信
  │    │    └─ registration timeout を延長
  │    └─ 001 受信 → 接続確立
  │
  ▼
  scheduleKeepaliveAlarm()
  storage.setAlarm(now + KEEPALIVE_INTERVAL)
       │
       │  ... KEEPALIVE_INTERVAL 経過 ...
       │
       ▼
  alarm() 発火
       ├─ サーバ接続中?
       │    ├─ YES → performConnectionHealthCheck()
       │    │         ├─ 通常時: scheduleKeepaliveAlarm() で再スケジュール
       │    │         ├─ 長時間無通信: 能動 PING を送信して再スケジュール
       │    │         └─ PING timeout: close() → onClose 側で再接続へ
       │    │
       │    └─ NO → autoReconnectOnDisconnect?
       │         ├─ YES → ensureServerConnection() 試行
       │         │    ├─ 成功 → keepalive サイクルに復帰
       │         │    └─ 失敗 → scheduleReconnectAlarm()（指数バックオフ + jitter）
       │         └─ NO → 何もしない（アラーム停止）
       │
       ▼
  次の alarm() 発火 → 上記を繰り返し
...
**重要**: keepalive アラームの主目的は DO を定期的に起こして eviction を防ぐことですが、現行実装では接続中に `performConnectionHealthCheck()` も呼ばれます。サーバーからの通常 `PING` には `pingModule` が `PONG` で応答し、能動 `PING` は「長時間サーバーから受信がない場合」にだけ送られます。
...
```
  ...
  └─ finalizeClose() 経由で onClose コールバック呼び出し
       │
       ▼
  handleServerClose(generation)
  ├─ stale generation? → 旧接続なら無視
  ├─ pendingRegistration を reject
  ├─ health-check 状態を初期化
  ├─ serverConn = null
  ├─ storage.deleteAlarm()          ← keepalive / reconnect アラーム停止
  ├─ connected 済み接続だった場合のみ:
  ...
  │         │    └─ scheduleReconnectAlarm()  ← 指数バックオフで再接続試行
  │         └─ それ以外 → 何もしない（停止状態）
  └─ 登録前に閉じた接続ならクライアント通知なしで次候補ポートへ
```
...
```
  ...
  ├─ resetConnectionRecoveryState()        ← reconnect 回数 / pending registration をリセット
...
```
  ...
  │    ├─ YES → ensureServerConnection() をバックグラウンド開始
  │    │    ├─ connectToServer() で新しい TCP 接続
  │    │    ├─ NICK / USER 送信
  │    │    ├─ 020 などの pre-registration 応答を待機
...
```
  ...
  refreshMessages() を1回実行
       │
       ▼
  旧ソケットの close を待たずに新しい /updates ソケットへ reconnect
       │
       ▼
  reconnect 後の fragment 再取得
  ├─ browser 側は `since=<latestSequence>` を付けて取得
  ├─ サーバ側が差分継続不能と判断したら `mode=full` を返す
  └─ browser 側は full 応答または sequence 不整合で全量置換してログ整合を取り直す
```
...
│  channels          参加中チャンネル名            │
│  channelSequences  最新 sequence キャッシュ      │
│  serverName        IRC サーバ名                  │
└─────────────────────────┘
```
...
同期: 接続直後に channel-updated を1回返して初期 sequence を共有
維持: 30秒ごとにブラウザが ping、DO が pong を返す
通知: 対象チャンネル更新時に最新 sequence 付き channel-updated を送信
...
継続: alarm() 発火 → 接続中なら health-check 実行後に再スケジュール
停止: 手動切断時に deleteAlarm() / サーバ切断時に deleteAlarm()
切替: サーバ切断 + autoReconnect=true → 再接続用アラーム（指数バックオフ + jitter）に移行
...
Web UI では、IRC メッセージ内または `POST /api/post` の `url` から URL プレビューを生成します。通常の画像・OGP カードに加えて、X/Twitter と YouTube は `kind: "rich"` を返す埋め込みプレビューを扱います。

### プレビュー解決フロー

URL preview の公開 API は [workers/src/modules/url-metadata.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/modules/url-metadata.ts) から見えますが、内部実装は `url-preview-policy.ts`、`url-preview-providers.ts`、`url-preview-resolver.ts` に分割されています。

`enableRemoteUrlPreview=true` のとき、次の 3 経路で `resolveMessageEmbed(text)` が呼ばれます。

```
PRIVMSG / NOTICE 受信                         自分の投稿 / API message
      │                                              │
      ▼                                              ▼
web-module.ts: buildTextMessage()      IrcProxyDO.postChannelMessage()
  └─ shouldResolveEmbed=true               └─ embed 未指定のとき resolveMessageEmbed(text) を実行
       │                                                    │
       └──────────────────────────┘
                              │
                              ▼
url-preview-resolver.ts: resolveUrlEmbed(url, { xTheme })
  ├─ 直リンク画像 → { kind: "image", imageUrl }
  ├─ YouTube → URL から動画 ID / 種別を抽出
  │    ├─ watch, youtu.be, /live, /embed → 通常動画
  │    ├─ /shorts → Shorts
  │    ├─ 固定 iframe HTML を生成
  │    │    ├─ width=355
  │    │    ├─ height=200 (通常動画)
  │    │    └─ height=631 (Shorts)
  │    ├─ YouTube oEmbed で title / provider_name を補完
  │    └─ { kind: "rich", html, title?, siteName }
  ...
  └─ その他 HTML → og:image / twitter:image / oEmbed を使って { kind: "card" }
```

### X/Twitter rich embed のテーマ決定

`theme` は保存済み Web UI 設定の `surfaceColor` から決めます。

```
webUiSettings.surfaceColor
      │
      ▼
resolveXEmbedTheme()
  ├─ #RRGGBB / #RGB を RGB に変換
  ├─ 輝度 = (R*299 + G*587 + B*114) / 1000
  ├─ 閾値未満 → dark
  └─ それ以外 / 不正値 → light
```

このため、同じ X URL でも Web UI 背景色が暗いチャンネル設定では `theme=dark`、明るい設定では `theme=light` で取得されます。

### 描画フロー

#### インライン表示 (`enableInlineUrlPreview=true`)

```
StoredMessage.embed.kind === "rich"
      │
      ▼
renderUrlEmbed(..., "inline")
  └─ .url-embed__rich-content に embed.html をそのまま描画
       ├─ X/Twitter: blockquote を配置
       │    └─ buildRichEmbedScript() が widgets.js を読み込み、X の埋め込みへ変換
       └─ YouTube: iframe をそのまま配置
```

apricot 側は X の埋め込み UI を再実装せず、`blockquote.twitter-tweet` を `widgets.js` で変換した結果をそのまま表示します。YouTube は `resolveYouTubeEmbed()` が生成した iframe をそのまま使用します。

#### ポップアップ表示 (`enableInlineUrlPreview=false`)

```
リンク hover / focus / long-press
      │
      ▼
buildPreviewScript()
  ├─ image/card → data-* 属性から既存 popup カードを組み立て
  └─ rich → hidden <template> を複製して popup に差し込む
```

rich embed の HTML は `data-*` に直接積まず、メッセージ行ごとの `<template>` に保持します。popup は視覚プレビュー専用で、外側コンテナは `pointer-events: none` のままです。

### rich embed の後処理

`buildRichEmbedScript()` はページ内の rich embed コンテナを走査し、X/Twitter の blockquote があれば `widgets.js` の読み込みと再描画を担当します。

```
メッセージ描画 / popup 表示
      │
      ▼
window.apricotRefreshRichEmbeds(root)
  ├─ root 配下に [data-apricot-rich-embed] があるか確認
  ├─ X/Twitter embed があれば widgets.js を lazy load
  └─ twttr.widgets.load(root) で blockquote を再変換
```

YouTube iframe には後処理は不要で、生成済みの `width="355"` と `height="200"` / `height="631"` をそのまま使います。CSS 側では `.url-embed__rich-content iframe { max-width: 100% !important; }` によりコンテナ幅へ収まるようにしています。

### REST API の投稿

#### `message` モード（`{ channel, message }`）

`enableRemoteUrlPreview=true` のとき [workers/src/irc-proxy.ts](/Users/nihondo/Library/CloudStorage/Dropbox/Projects.localized/apricot/workers/src/irc-proxy.ts) の `postChannelMessage()` 内で `resolveMessageEmbed(message)` が実行され、embed が生成されます。

```
POST /api/post { channel, message }
      │
      ▼
postChannelMessage(channel, message, embed=undefined)
  └─ enableRemoteUrlPreview=true のとき resolveMessageEmbed(message) を実行
       └─ 解決済み embed を recordSelfMessage() に渡す
```

#### `url` モード（`{ channel, url }`）

Web UI 保存用の preview 解決と、IRC に実際に流す投稿テキスト生成は別経路で処理します。このモードは `enableRemoteUrlPreview` の値に関わらず常にメタデータ取得を行い、embed を `postChannelMessage` に渡します（`postChannelMessage` 内の自動解決はスキップされます）。

```
POST /api/post { channel, url }
      │
      ├─ url-preview-resolver.ts: resolveUrlEmbed(url, { xTheme: resolveXEmbedTheme(webUiSettings.surfaceColor) })
      │    └─ Web UI ログ保存用の embed を生成
      └─ url-preview-resolver.ts: extractUrlMetadata(url, { browserRendering })
           └─ IRC 投稿用テキストを生成
```
...
このため、Web UI から URL 単体投稿した場合も、保存済みテーマに合わせた X rich embed や YouTube iframe embed は message log に残りつつ、IRC に実際に送る投稿文のタイトル解決は `extractUrlMetadata()` 側で独立して処理されます。一般 URL の IRC 投稿文だけは Cloudflare Browser Rendering のシークレット設定があればレンダリング後タイトルを優先できます。
