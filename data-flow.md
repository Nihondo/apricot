...
```
  ...
  ├─ reconnect 後の最初の channel-updated は revision 比較をスキップして必ず refreshMessages()
  └─ 以降は新しい revision 系列で通常同期に復帰
```
...
```
  ...
  旧ソケットの close を待たずに新しい /updates ソケットへ reconnect
       │
       ▼
  reconnect 後の最初の channel-updated
  ├─ browser 側の古い apricotLatestRevision と比較しない
  ├─ 通知側 revision を新しい系列の起点として採用
  └─ 必ず refreshMessages() を実行してログ整合を取り直す
```
