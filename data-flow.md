...
Web UI では、IRC メッセージ内または `POST /api/post` の `url` から URL プレビューを生成します。通常の画像・OGP カードに加えて、X/Twitter URL は `oEmbed.html` を使った rich embed を扱います。
...
`srcdoc` 内では `widgets.js` が `blockquote.twitter-tweet` を X の埋め込み iframe に変換します。apricot 側は X 互換 UI を再実装せず、X の描画結果をそのまま表示します。
...
X の最終埋め込み高さは `widgets.js` 実行後でないと確定しないため、outer iframe の高さは後追いで更新します。

```
srcdoc 内
  ├─ ResizeObserver / MutationObserver / setTimeout で高さ監視
  └─ window.parent.postMessage({
       type: "apricot-x-embed-resize",
       embedId,
       height
     }, "*")

親ページ
  └─ buildRichEmbedScript() が message を受信
       └─ data-apricot-rich-embed-id 一致の iframe 高さを更新
```

`maxheight=200` は oEmbed API には渡しますが、最終高さの制御には使いません。実際にレンダリングされた高さを outer iframe に反映します。
