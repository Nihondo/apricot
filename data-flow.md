...
```
  ...
  ├─ cloudflare:sockets connect(host, port) で Socket 作成
  ├─ socket.opened を待つ  ← IRC_CONNECT_TIMEOUT_MS
  ...
  ├─ resolvePendingRegistration()  ← 001 が来るまで connectToServer は成功扱いにしない
  ├─ serverName = 応答から取得
...
```
  ...
  alarm() 発火
       ├─ サーバ接続中?
       │    ├─ YES → health check を評価
       │    │         ├─ 最近メッセージ受信あり → scheduleKeepaliveAlarm()
       │    │         ├─ 無通信が IRC_IDLE_PING_INTERVAL_MS 超過 → PING apricot:<timestamp>
       │    │         └─ PONG が IRC_PING_TIMEOUT_MS 以内に来ない → close() して通常の再接続フローへ
       │    │
       │    └─ NO → autoReconnectOnDisconnect?
       │         ├─ YES → ensureServerConnection() 試行
       │         │    ├─ 成功 → 通常の keepalive サイクルに復帰
       │         │    └─ 失敗 → scheduleReconnectAlarm() (指数 backoff + jitter)
       │         └─ NO → 何もしない（アラーム停止）
       │
       ▼
  次の alarm() 発火 → 上記を繰り返し
...
**重要**: keepalive アラームの主目的は DO を定期的に起こして eviction を防ぐことですが、現在は同時に接続 health check の観測点も兼ねています。通常の IRC `PING` への受動 `PONG` は `pingModule` が処理し、無通信時の能動 `PING` は `IrcProxyDO.alarm()` から送られます。
...
```
  ...
  └─ consumeAutoReconnectOnClose()
       ...
       │    └─ scheduleReconnectAlarm()  ← 指数 backoff + jitter で再接続試行
       │
       └─ それ以外 → 何もしない（停止状態）
```
...
継続: alarm() 発火 → 接続中なら health check を評価したうえで再スケジュール
停止: 手動切断時に deleteAlarm() / サーバ切断時に deleteAlarm()
切替: サーバ切断 + autoReconnect=true → 再接続用アラーム (指数 backoff + jitter) に移行
...
stateDiagram-v2
    ...
    Connecting --> Connected: socket.opened + 001 RPL_WELCOME<br/>keepalive 開始
    ...
    ReconnectWait --> Connecting: backoff 後アラーム発火

    state Connected {
        [*] --> KeepAlive
        KeepAlive --> KeepAlive: alarm() → health check / 再スケジュール
