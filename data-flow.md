...
│  │  - web store (msg buf + sequence) - webUpdateSubscribers                                                │    │
│  └─────────────────────────────────────────────────────┘    │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ DO Storage (永続化)                                                                                      │    │
...
PRIVMSG / NOTICE 受信                自分の投稿 / API message
      │                                     │
      ▼                                     ▼
web module: buildTextMessage()      IrcProxyDO: postChannelMessage()
...
`enableRemoteUrlPreview=true` のとき `postChannelMessage()` 内で `resolveMessageEmbed(message)` が実行され、embed が生成されます。
