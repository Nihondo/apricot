...
| `IRC_AUTO_CONNECT_ON_STARTUP` | ─ | `false` | DO 起動時に自動接続 |
| `IRC_AUTO_RECONNECT_ON_DISCONNECT` | ─ | `false` | 切断時に自動再接続。固定間隔ではなく指数 backoff + jitter で継続再試行 |
| `IRC_CONNECT_TIMEOUT_MS` | ─ | `10000` | TCP ソケット open 完了待ちタイムアウト（ミリ秒） |
| `IRC_REGISTRATION_TIMEOUT_MS` | ─ | `20000` | PASS / NICK / USER 後に `001` を待つタイムアウト（ミリ秒） |
...
| `IRC_IDLE_PING_INTERVAL_MS` | ─ | `240000` | サーバ受信が止まった接続へ能動 `PING` を送るまでの無通信時間（ミリ秒） |
| `IRC_PING_TIMEOUT_MS` | ─ | `90000` | 能動 `PING` の応答待ちタイムアウト。超過時は切断して再接続へ流す（ミリ秒） |
| `IRC_AUTOJOIN` | ─ | ─ | 自動参加チャンネル（カンマ区切り） |
| `IRC_ENCODING` | ─ | `utf-8` | 文字コード |
| `ENABLE_REMOTE_URL_PREVIEW` | ─ | `false` | URL 自動プレビュー取得の総合スイッチ（受信・自分の投稿に適用） |
| `KEEPALIVE_INTERVAL` | ─ | `60` | キープアライブ間隔（秒）。DO の生存維持と health check の観測粒度を兼ねる |
...
Cloudflare のデプロイやランタイム更新時に Durable Object が再生成されることがあります。`IRC_AUTO_RECONNECT_ON_DISCONNECT = "true"` を設定しておくと、自動的に再接続されます。再接続は固定 5 秒ではなく、`IRC_RECONNECT_BASE_DELAY_MS` から始まる指数 backoff + jitter で継続再試行されます。

長時間メッセージが来ない接続では、`KEEPALIVE_INTERVAL` ごとの alarm で health check が評価され、必要に応じて `PING` / `PONG` による生存確認が走ります。`KEEPALIVE_INTERVAL` を長くすると切断検知も遅くなる点に注意してください。
