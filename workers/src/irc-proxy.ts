...
export class IrcProxyDO implements DurableObject {
  ...
  constructor(state: DurableObjectState, env: Env) {
    ...
    this.registrationTimeoutMs = parseIntegerEnv(env.IRC_REGISTRATION_TIMEOUT_MS, 120_000);
  ...
  private async handleServerMessage(
    ...
    this.lastServerActivityAt = activityAt;

    if (this.pendingRegistration?.generation === generation && cmd !== "001") {
      this.refreshPendingRegistration(generation, activityAt);
      console.log(`IRC: pre-registration message generation=${generation} command=${cmd}`);
    }
  ...
  private handleStartupAutoConnect(): void {
    ...
    void this.ensureServerConnection().catch((error) => {
      console.error("Failed to auto-connect on startup", error);
      if (this.config?.autoReconnectOnDisconnect) {
        void this.scheduleReconnectAlarm();
      }
    });
  }
  ...
  private async scheduleReconnectAlarm(): Promise<void> {
    ...
    const nextReconnectDelayMs = this.calculateReconnectDelayMs(this.reconnectAttempt);
    console.log(`IRC: scheduling reconnect attempt=${this.reconnectAttempt} delayMs=${nextReconnectDelayMs}`);
    await this.state.storage.setAlarm(Date.now() + nextReconnectDelayMs);
  }
  ...
  /**
   * Extends registration timeout while the server is still sending pre-registration responses.
   */
  private refreshPendingRegistration(generation: number, activityAt: number): void {
    if (!this.pendingRegistration || this.pendingRegistration.generation !== generation) {
      return;
    }

    clearTimeout(this.pendingRegistration.timer);
    this.pendingRegistration.lastActivityAt = activityAt;
    this.pendingRegistration.timer = setTimeout(() => {
      this.rejectPendingRegistration(
        generation,
        new Error(`registration timed out after ${this.registrationTimeoutMs}ms of inactivity`),
      );
    }, this.registrationTimeoutMs);
  }
